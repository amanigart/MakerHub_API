...
BEGIN
	...
	IF NOT EXISTS (SELECT [login] FROM AppUtilisateur WHERE [login] = @login AND passwd = @passwordHash)
		-- THROW 51001, 'Mot de passe incrorrect.', 1
		THROW 51001, @passwordHash, 1

	-- Sortir utilisateur si login + passwd corrects
	SELECT id_utilisateur as idUtilisateur, [login]
	FROM AppUtilisateur 
