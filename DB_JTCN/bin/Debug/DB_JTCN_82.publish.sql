...
GO
PRINT N'Création de Vue [dbo].[V_TopCeinutres]...';


GO
CREATE VIEW [dbo].[V_TopCeinutres]
	AS 
	SELECT cm.idMembre, d.nom as discipline, c.couleur as ceinture
	FROM Discipline as d 
		 JOIN Ceinture as c ON (d.idDiscipline = c.idDiscipline)
		 JOIN CeintureMembre as cm ON (c.idCeinture = cm.idCeinture)
	WHERE (d.idDiscipline = 1) OR (d.idDiscipline = 2)
GO
...
END
GO
PRINT N'Création de Procédure [dbo].[GetTopCeinturesByMembre]...';


GO
CREATE PROCEDURE [dbo].[GetTopCeinturesByMembre]
	@idMembre INT
AS
BEGIN

	SELECT TOP 2 d.nom, c.couleur
	FROM Discipline as d 
			JOIN Ceinture as c ON (d.idDiscipline = c.idDiscipline)
			JOIN CeintureMembre as cm ON (c.idCeinture = cm.idCeinture)
	WHERE (cm.idMembre = @idMembre AND d.idDiscipline = 1) OR (cm.idMembre = @idMembre AND d.idDiscipline = 2)
	ORDER BY cm.id Desc
